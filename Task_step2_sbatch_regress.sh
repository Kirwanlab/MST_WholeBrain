...
deconLen=(3.5)										# trial duration for each Phase (argument for BLOCK in deconvolution)
deconPref=(mst)				            			# array of prefix for each planned decon (length must equal sum of $deconNum)
...
arrmst=(CR Hit LureFA LureCR Other)						# arrFoo matches a $deconPref value, one string per .1D file (e.g. arrSpT1=(Hit CR Miss FA))
